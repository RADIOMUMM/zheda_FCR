...
def plot_weight_curve_stage(result, output_path, stage_start, stage_end, show_fit=True, return_fig=False, ax=None):
    """绘制阶段体重曲线（使用已质控数据，标注阶段范围）"""
    import matplotlib.dates as mdates
    df = result.cleaned_data
    if df is None or len(df) == 0:
        return None
    ...
    # 全部有效体重（灰色小点）
    all_valid = df[~df["is_abnormal"] & df["qc_weight_kg"].notna()]
    if len(all_valid) > 0:
        ax.scatter(all_valid["visit_time"], all_valid["qc_weight_kg"],
                   c="#cccccc", s=10, alpha=0.3, zorder=1, label="全期有效体重")

    # 阶段有效体重（蓝色大点）
    stage = all_valid[(all_valid["qc_weight_kg"] >= stage_start) & (all_valid["qc_weight_kg"] <= stage_end)]
    if len(stage) > 0:
        ax.scatter(stage["visit_time"], stage["qc_weight_kg"],
                   c="#3498db", s=35, alpha=0.8, zorder=3, label=f"阶段体重({stage_start}-{stage_end}kg)")

    # 异常体重
    abnormal = df[df["is_abnormal"] & df["weight_kg"].notna()]
    if len(abnormal) > 0:
        ax.scatter(abnormal["visit_time"], abnormal["weight_kg"],
                   c="#e74c3c", s=15, alpha=0.3, marker="x", zorder=2, label="异常体重")

    # 阶段拟合线
    if show_fit and len(stage) >= 3:
        x_min = stage["visit_time"].min()
        x_days = (stage["visit_time"] - x_min).dt.days.values.astype(float)
        y_vals = stage["qc_weight_kg"].values
        A = np.vstack([x_days, np.ones(len(x_days))]).T
        m, c = np.linalg.lstsq(A, y_vals, rcond=None)[0]
        x_fit = np.linspace(0, x_days.max(), 100)
        y_fit = m * x_fit + c
        fit_dates = [x_min + pd.Timedelta(days=int(d)) for d in x_fit]
        ax.plot(fit_dates, y_fit, "-", color="#e74c3c", linewidth=2.5, zorder=4,
                label=f"阶段ADG={m:.4f} kg/d")

    # 阶段范围标注（横向虚线标尺）
    ax.axhline(y=stage_start, color="#2ecc71", linestyle="--", alpha=0.7, linewidth=1)
    ax.axhline(y=stage_end, color="#2ecc71", linestyle="--", alpha=0.7, linewidth=1)
    ax.annotate(f"{stage_start}kg", xy=(0.01, stage_start), fontsize=9, color="#2ecc71",
                transform=ax.get_yaxis_transform(), va="bottom")
    ax.annotate(f"{stage_end}kg", xy=(0.01, stage_end), fontsize=9, color="#2ecc71",
                transform=ax.get_yaxis_transform(), va="bottom")

    info = f"{result.lifenumber} | 阶段FCR:{getattr(result, 'stage_fcr', None) or '-'} | ADG:{getattr(result, 'stage_adg', None) or '-'}"
    ax.set_title(info, fontsize=13, fontweight="bold")
    ax.set_xlabel("日期"); ax.set_ylabel("体重 (kg)")
    ax.xaxis.set_major_formatter(mdates.DateFormatter("%m-%d"))
    fig.autofmt_xdate()
