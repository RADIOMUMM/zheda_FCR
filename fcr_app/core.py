...
def filter_individual(df: pd.DataFrame, min_age_days: int = 100, max_age_days: int = 200) -> Tuple[pd.DataFrame, List[str]]:
    ...
    # 单次记录过滤：剔除明显异常的体重值
    df = df[(df["weight_kg"] >= 25) & (df["weight_kg"] <= 180)].copy()
    return df, info
...
def calc_adfi(df: pd.DataFrame) -> Optional[float]:
    """计算平均日采食量 — 使用全部原始采食量总和 / 测试天数"""
    total = df["feed_kg"].sum()
...
def process_pig(df_raw: pd.DataFrame, lifenumber: str, min_age: int = 100, max_age: int = 200, init_lower: float = 25.0, init_upper: float = 45.0) -> PigResult:
    ...
    # 提取起止信息
    result.start_date = df["visit_time"].min().date()
    result.end_date = df["visit_time"].max().date()
    result.start_weight = round(df["weight_kg"].iloc[0], 1)
    result.end_weight = round(df["weight_kg"].iloc[-1], 1)
    ...
    # 总采食量
    result.total_feed = round(df["feed_kg"].sum(), 3)

    # 原始FCR（用起止体重差替代ADG）
    if adg and adg > 0:
        days = (df["visit_time"].max() - df["visit_time"].min()).days
        if days > 0:
            result.fcr_raw = round(df["feed_kg"].sum() / (adg * days), 3)
...
def batch_process(filepath: str, output_dir: str,
                  progress_callback=None, init_lower: float = 25.0, init_upper: float = 45.0) -> List[PigResult]:
    """批量处理所有猪只"""
    ...
    for i, (lifenumber, group) in enumerate(pig_groups):
        ...
        result = process_pig(group.copy(), lifenumber, init_weight_min=init_weight_min)
